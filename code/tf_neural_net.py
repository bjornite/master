...
class CBTfTwoLayerNet(object):
    def __init__(self, input_size, output_size, log_dir="test_logs"):
        ...
        with tf.name_scope('error_prediction_layer'):
            WEP = tf.Variable(
                tf.random_normal([self.n_input, 1],
                                 stddev=np.sqrt(2.0 / self.n_input)), name='WP')
            bEP = tf.Variable(tf.constant(0.1, shape=[1]), name='bP')
        ...
        self.error_train_op = optimizer.minimize(self.error_prediction_loss,
    ...
    def train(self, x, x_next, targetQ, targetActionMask):
        ...
        feed_dict = {self.X: x,
                     ...
                     self.targetQ: targetQ,
                     self.targetActionMask: targetActionMask}
        opt, summary, global_step, meta_error = self.sess.run([self.train_op,
                                                               self.merged,
                                                               self.global_step,
                                                               self.error_train_op],
                                                              feed_dict=feed_dict)
    ...
    def get_meta_prediction_error(self, x, x_next):
        feed_dict = {self.X: x,
                     self.X_next: x_next}
